...
'
'その他注意点
'1. 再帰関数には対応していない為､実装しない事｡どうしてもやる場合コールスタックの閾値を外す
'2. 関数呼び出し時にOn Error Resume Next等叩いてハンドラを無視している場合、呼び出し前にOn Error GoTo ErrorHandlerに復帰させておく事
'----------------------------------------------------------------------------------------------------+
...
    
    ...
    errorText = "==================================================" & vbCrLf & _
                ...
                "  StackTrace  : " & vbCrLf & strStackTrace & vbCrLf
    
    'ログファイルに出力
    fileNumber = FreeFile
...
    
        ...
        'スタックトレース作成の場合
        Else
            tmpStack = "    " & Format(1 + removeCount, "@@@@@") & " " & stackList(1)
            For i = 2 To stackList.Count
                tmpStack = tmpStack & vbCrLf & "    " & Format(i + removeCount, "@@@@@") & " " & stackList(i)
            Next i
            '古いスタックトレースの履歴を削除した場合、スタックトレースの末尾に件数を記述
            If removeCount > 0 Then
                tmpStack = tmpStack & vbCrLf & _
                "  *-設定された履歴件数の閾値（" & MAX_STACK_COUNT & "件）を超えた為、開始から" & removeCount & "件の履歴が削除されました。-*"
            End If
        End If
    End If
    
