...
import os, threading, time, csv, sys
...
class App:
    ...
    def build_ui(self):
        ...
        tk.Radiobutton(f,text="右",variable=self.key_var,value="Right").grid(row=0,column=1)
        tk.Radiobutton(f,text="左",variable=self.key_var,value="Left").grid(row=0,column=2)
        ...
        tk.Checkbutton(f,text="画面分割",variable=self.split_var,command=self.update_split_state).grid(row=0,column=0)
        ...
        # 座標
        tk.Button(self.root,text="座標選択",command=self.set_coords,width=12).grid(row=row,column=0,sticky="w",padx=5)
        self.coord_label = tk.Label(text="未設定", width=40, anchor="w")
        self.coord_label.grid(row=row,column=1,columnspan=3,sticky="w")
        row += 1

        # ウィンドウ
        tk.Button(self.root,text="ウィンドウ選択",command=self.select_window,width=12).grid(row=row,column=0,sticky="w",padx=5)
        ...
        self.progress.grid(row=row,column=0,columnspan=3,sticky="we",padx=5)
        self.progress['value'] = 0
        # 停止ボタン
        self.stop_btn = tk.Button(self.root,text="停止",command=self.stop_process,state="disabled")
        self.stop_btn.grid(row=row,column=3,padx=5)
        row += 1
    ...
    def run_process(self):
        ...
        cover = self.cover_entry.get() if self.cover_var.get() and os.path.exists(self.cover_entry.get()) else None
        if self.running:
